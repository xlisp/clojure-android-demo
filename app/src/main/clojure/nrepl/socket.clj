...
(def unix-domain-flavor
  (cond
    ;; Android patch: on Dalvik, java.net.UnixDomainSocketAddress and the
    ;; java.nio ProtocolFamily/UNIX socket internals are either absent or
    ;; blocked hidden APIs, so eagerly initializing JDK unix-domain-socket
    ;; support at load time crashes nrepl.socket (and thus nrepl.server).
    ;; We only ever serve nREPL over TCP here, so force the flavor to nil.
    (= "Dalvik" (System/getProperty "java.vm.name")) nil
    test-junixsocket? (do
